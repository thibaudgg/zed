...
const INFERENCE_FORBIDDEN_MESSAGE: &str = "Login succeeded, but SuperGrok inference was forbidden \
    (HTTP 403). Standard SuperGrok sometimes cannot use this API. SuperGrok Heavy is more reliable. \
    You can also use the separate xAI provider with an API key from console.x.ai.";
...
impl SuperGrokModel {
    ...
    pub fn max_token_count(&self) -> u64 {
        match self {
            Self::Grok46 | Self::Grok45 => 500_000,
            Self::GrokBuild01 => 256_000,
    ...
    pub fn max_output_tokens(&self) -> Option<u64> {
        match self {
            Self::Grok46 | Self::Grok45 | Self::GrokBuild01 => Some(64_000),
            Self::Custom {
                max_output_tokens, ..
            } => *max_output_tokens,
        }
    ...
    pub fn supports_images(&self) -> bool {
        match self {
            Self::Grok46 | Self::Grok45 | Self::GrokBuild01 => true,
            Self::Custom {
    ...
    pub fn supports_reasoning_effort(&self) -> bool {
        match self {
            Self::Grok46 | Self::Grok45 => true,
            // grok-build-0.1 rejects the reasoningEffort parameter entirely.
            Self::GrokBuild01 => false,
            Self::Custom {
...
fn advertised_reasoning_efforts(model: &SuperGrokModel) -> &'static [ReasoningEffort] {
    ...
    // xAI rejects `reasoning_effort: "none"` on grok-4.6. Compact and title
    // requests disable thinking, so we omit the field instead of sending none.
    if model.supports_xhigh_effort() {
        &[
            ReasoningEffort::Low,
...
fn map_completion_error(error: LanguageModelCompletionError) -> LanguageModelCompletionError {
    match error {
        LanguageModelCompletionError::PermissionError { provider, .. }
...
mod tests {
    ...
    #[test]
    fn grok_46_supports_xhigh_and_defaults_to_medium() {
