...
use x_ai_subscribed::{
    PROVIDER_ID, PROVIDER_NAME, State, SuperGrokModel, XAI_API_URL, create_language_model,
};

use crate::AllLanguageModelSettings;

const SUBSCRIPTION_DESCRIPTION: &str =
    "Sign in with your SuperGrok or SuperGrok Heavy subscription to use Grok in Zed's agent.";

#[derive(Default, Clone, Debug, PartialEq)]
pub struct XAiSubscribedSettings {
    pub api_url: String,
    pub available_models: Vec<settings::XaiAvailableModel>,
...
impl XAiSubscribedProvider {
    ...
    fn settings(cx: &App) -> &XAiSubscribedSettings {
        &AllLanguageModelSettings::get_global(cx).x_ai_subscribed
    }
    ...
    fn models(cx: &App) -> Vec<SuperGrokModel> {
        let mut models = BTreeMap::default();
        for model in SuperGrokModel::all() {
            models.insert(model.id().to_string(), model);
        }
        for model in &Self::settings(cx).available_models {
            models.insert(
                model.name.clone(),
                SuperGrokModel::Custom {
    ...
    fn create_model(&self, model: SuperGrokModel, cx: &App) -> Arc<dyn LanguageModel> {
        create_language_model(
            ...
            &self.state,
            Self::api_url(cx),
            Self::settings(cx).custom_headers.clone(),
            cx,
        )
    }
}
